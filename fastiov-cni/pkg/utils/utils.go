...
func saveScratchNetConf(containerID, dataDir string, netconf []byte) error {
	...
	if DoDebugDoubleCmdDel {
		debug.Logf("save net conf to %s ok", path)
		exists, err := fileExists(path)
		if err != nil {
			debug.Logf("Failed to check if file exists: %v\n", err)
			return err
		}

		if exists {
			err = enableAuditCtlFileMonitor(path)
			if err == nil {
				debug.Logf("The file at path %s exists, add auditctl ok\n", path)
			} else {
				debug.Logf("The file at path %s exists, add auditctl error\n", path)
			}
		} else {
			debug.Logf("The file at path %s does not exist.\n", path)
		}
	}
	return err
}
...
func enableAuditCtlFileMonitor(path string) error {
	cmd := exec.Command("auditctl", "-w", path, "-p", "wa")
	if err := cmd.Start(); err != nil {
		return fmt.Errorf("failed to start auditctl: %w", err)
	}
	return nil
}
...
func ReadScratchNetConf(cRefPath string) ([]byte, error) {
	if DoDebugDoubleCmdDel {
		exists, err := fileExists(cRefPath)
		if err != nil {
			debug.Logf("Failed to check if file exists: %v\n", err)
			return nil, err
		}
		...
		if exists {
			debug.Logf("The file at path %s exists.", cRefPath)
		} else {
			debug.Logf("The file at path %s does not exist.\n", cRefPath)
		}
	}
	...
	if err != nil {
		if DoDebugDoubleCmdDel {
			debug.Logf("cannot find net conf in %s!!!", cRefPath)
		}
