...
func getAllVfInfoIDStr(link netlink.Link) string {
	vfs := link.Attrs().Vfs
	numVfs := len(vfs)
	infoIDStr := fmt.Sprintf("[%d]VFs:(", numVfs)
	for idx, vf := range vfs {
		if idx == numVfs-1 {
			infoIDStr += fmt.Sprintf("%d)", vf.ID)
		} else {
			infoIDStr += fmt.Sprintf("%d,", vf.ID)
		}
	}
...
func (s *sriovManager) FillOriginalVfInfo(conf *sriovtypes.NetConf) error {
	...
	debug.Logf("[sriov-cni] listing pf (%s): %s", conf.Master, getAllVfInfoIDStr(pfLink))
