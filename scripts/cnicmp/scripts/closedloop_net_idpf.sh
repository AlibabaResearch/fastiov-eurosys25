...
client() { # client sandbox.yaml output_filename
    ...
    #wait for eth0 to be probed by idpf driver
    while true; do
        if bash /root/ipu/scripts/fastiov_check_eth0.sh $pid | grep -q "yes"; then
            break
        else
            sleep 0.1
        fi
    done
    local end_time2=$($TIME_MILLI)

    local total_latency=$(($end_time2 - $start_time))
    local netdriver_latency=$(($end_time2 - $end_time))
    echo $pid $start_time $end_time $total_latency >> $output
    echo $pid $start_time $end_time $end_time2 $total_latency $netdriver_latency >> /root/ipu/kata-netdriver.log
}
