...
sed -r -i "s/default_memory = [0-9]+/default_memory = 2048/g" $kata_config
...
for test_concurrency in ${concurency[@]}; do
    if [ $(( $test_concurrency % $test_tenants )) -eq 0 ]; then
        ...
        $DIR/closedloop_net.sh -c $test_concurrency -t $test_tenants -i $test_iters -p kata $remove_flag $no_network_flag $no_clean_flag
        if [[ $? != 0 ]]; then
            exit $?
        fi
    else
