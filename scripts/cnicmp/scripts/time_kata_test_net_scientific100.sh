...
for test_concurrency in ${concurency[@]}; do
    if [ $(( $test_concurrency % $test_tenants )) -eq 0 ]; then
        ...
        $DIR/closedloop_app_scientific100.sh -c $test_concurrency -t $test_tenants -i $test_iters -p kata $remove_flag $no_network_flag $no_clean_flag
        if [[ $? != 0 ]]; then
            exit $?
        fi
    else
